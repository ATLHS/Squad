  ...
  Mentor.findOne({ email }, (err, user) => {
    ...
    newMentor.save((err, user) => {
      ...
      transporter.sendMail({
        ...
        context: { name, firstname, email, live_name, bio, price },
      });
      res.json({
        success: true,
        message: `Merci pour votre inscription ${name}, une personne de la Squad team vous contactera d\'ici peux. kiss !`,
      });
    });
  });
});
