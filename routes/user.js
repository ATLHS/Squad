...
transporter.use('compile', hbs({
    ...
    transporter.sendMail({
        ...
        template: 'live_reservation_alert',
        context: {
            name,
            email,
            numberOfUsers,
    ...
    Mentor.findOne({email}, (err, user) => {
        ...
        newMentor.save((err, user) => {
            ...
            transporter.sendMail({ 
                ...
                template: 'mentor_signup',
                context: { firstname }
            })
            // Send email to Squad team   
            transporter.sendMail({
                from: '"Squad" <contact@squadapp.fr>',
                to: "s-attilah@hotmail.com",
                subject: "Inscription : Mentor",
                template: 'mentor_signup_alert',
                context: { name, firstname, email, tel }
            })
            res.json({ success: true, message: `Merci pour votre inscription ${name}, une personne de la Squad team vous contactera d\'ici peux. kiss !`})
        })
    })
